...
export default class Login extends Component {
  ...
  */
  componentDidMount() {
    ...
    Animated.timing(this.state.fade_animation, {
      toValue: 1,
      duration: 1000
    }).start();
  }
  ...
  toggleSignUp() {
    ...
    Animated.timing(this.state.fade_animation, {
      ...
      if (!that.state.sign_up) {
        that.setState({page_text: "Sign in", button_text: "sign in!"});
      } else {
        that.setState({page_text: "Sign up", button_text: "sign up!"});
      }
      Animated.timing(that.state.fade_animation, {
        toValue: 1,
        duration: 500
      }).start();
    });
  ...
  */
  signin = () => {
    if ((!this.state.email)) {
      ...
      this.refs.valert.showAlert('Woah there!', 'You can\'t log in with an empty email!', 'I understand');
      return;
    }
    if ((!this.state.password)) {
      this.refs.submitButton.indicateError();
      this.refs.valert.showAlert('Hey there, friendo!', 'You can\'t log in with an empty password!', 'I understand');
      return;
    }
    this.openModal();
    var that = this;
    Auth.firebaseLogin(this.state.email, this.state.password).then(function(rv) {
      ...
      that.closeModal();
      that.refs.valert.showAlert('Alert', error.message, 'Ok');
    })
  }
  ...
  */
  signup = () => {
    if ((!this.state.email)) {
      this.refs.submitButton.indicateError();
      this.refs.valert.showAlert('Now wait just a second!', 'You can\'t sign up with an empty email!', 'I understand');
      return;
    }
    if ((!this.state.password)) {
      this.refs.submitButton.indicateError();
      this.refs.valert.showAlert('Hold up!', 'You can\'t sign up with an empty password!', 'I understand');
      return;
    }
    if (this.state.password != this.state.password_verification) {
      this.refs.submitButton.indicateError();
      this.refs.valert.showAlert('Our OCD is going off...', 'your passwords don\'t match', 'I understand');
      return;
    }
    this.openModal();
    var that = this;
  ...
  render() {
    ...
    */
    var pw_confirm_field = this.state.sign_up
      ? <InputField ref="password2" icon={Icons.check} label={"re-enter password"} labelColor={"rgba(37,50,55,0.5)"} inactiveColor={GLOBAL.COLOR.DARKGRAY} activeColor={GLOBAL.COLOR.GREEN} topMargin={24} autoCapitalize={"none"} secureTextEntry={true} autoCorrect={false} returnKeyType={'go'} onChangeText={(text) => {
            this.setState({password_verification: text})
          }
    ...
    */
    ...
    return (<SafeAreaView style={[styleguide.container, styles.container]}>
      ...
      <Animated.View style={[
        ...
        <Animated.Text style={[
            styleguide.light_display2, {
              fontSize: this.state.pageTextSize
            }
          ]}>
          {this.state.page_text}
          <Animated.Text style={[
              styleguide.light_display2_accent, {
                fontSize: this.state.pageTextSize
              }
            ]}>.</Animated.Text>
        </Animated.Text>
        <InputField icon={Icons.inbox} label={"email"} labelColor={"rgba(37,50,55,0.5)"} inactiveColor={GLOBAL.COLOR.DARKGRAY} activeColor={GLOBAL.COLOR.GREEN} topMargin={this.state.formMargin} autoCapitalize={"none"} keyboardType={"email-address"} autoCorrect={false} returnKeyType={'next'} onChangeText={(text) => {
            this.setState({email: text})
          }} onSubmitEditing={() => {
            this.refs.password1.focus();
          }}/>
        <InputField ref="password1" icon={Icons.lock} label={"password"} labelColor={"rgba(37,50,55,0.5)"} inactiveColor={GLOBAL.COLOR.DARKGRAY} activeColor={GLOBAL.COLOR.GREEN} topMargin={24} autoCapitalize={"none"} secureTextEntry={true} autoCorrect={false} returnKeyType={this.state.sign_up
            ? 'next'
            : 'go'} onChangeText={(text) => {
        ...
        <TouchableOpacity onPress={() => {
          ...
          <Text style={[styleguide.light_body, styles.forgot_password_text]}>forgot password?</Text>
        </TouchableOpacity>
        <Button ref="submitButton" backgroundColor={GLOBAL.COLOR.GREEN} label={this.state.button_text} height={64} marginTop={40} shadow={true} onPress={() => {
            if (this.state.sign_up) {
              this.signup();
            } else {
              this.signin();
