...
export default class ForgotPassword extends Component {
  ...
  passwordReset = () => {
    if (!this.state.email) {
      ...
      this.refs.valert.showAlert('No email entered', 'Simply enter your email address and try again!', 'Ok');
    } else {
      var that = this;
      Auth.firebasePasswordReset(this.state.email).then(function() {
        that.refs.linkButton.indicateError();
        that.refs.valert.showAlert('Password Reset Request Received', 'Please check your email', 'I\'ll go check!', GLOBAL.COLOR.GREEN);
  ...
  render() {
    ...
    return (<SafeAreaView style={[
      ...
      <Animated.View style={[
        ...
        <Animated.Text style={[
            styleguide.light_subheader_secondary, {
        ...
        <InputField icon={Icons.inbox} label={"email"} labelColor={"rgba(37,50,55,0.5)"} inactiveColor={GLOBAL.COLOR.DARKGRAY} activeColor={GLOBAL.COLOR.GREEN} topMargin={this.state.topMargin} autoCapitalize={"none"} keyboardType={"email-address"} autoCorrect={false} onChangeText={(text) => {
            this.setState({email: text})
          }}/>
        <Button ref="linkButton" backgroundColor={GLOBAL.COLOR.GREEN} label={"send me a link"} height={64} marginTop={40} width={'100%'} shadow={true} onPress={() => {
            this.passwordReset();
          }}/>
      </Animated.View>
    </SafeAreaView>);
